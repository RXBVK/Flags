...
import { shuffleArray, getAnswers, handleTime } from "../../../utils/utils";
...
export default function FlagToCountry({ allCountries }) {
  ...
  const startTheGame = () => {
    ...
    setGameStarted(true);
    getQuestion();
    manageTimer("on", Date.now());
  };

  const manageTimer = (mode, timeWhenClicked) => {
    const interval = setInterval(() => {
      let time = Date.now();
      let proper = Math.floor((time - timeWhenClicked) / 1000);
      handleTime(proper);
      setGameTime(proper);
    }, 1000);
    if (mode === "off") {
  ...
  const handleAnswer = (e) => {
    ...
    // if (availableCountries.length === 0 || !answerWasGood) {
    //   setGameResult(answerWasGood);
    // }
  };
